...
async function load(url, cb) {
    try {
        ...
        const html = res.data;
        const doc = await parseHtml(html);
        const title = doc.querySelector('h1')?.textContent.trim() || doc.querySelector('.film-name')?.textContent.trim();
        const description = doc.querySelector('.description')?.textContent.trim() || doc.querySelector('.entry-content')?.textContent.trim();
        ...
        const genreEls = Array.from(doc.querySelectorAll('.genre a, .genres a, .category a, .sgenerx a')).map(el => el.textContent.trim()).filter(Boolean);
