...
async function load(url, cb) {
    try {
        ...
        const description = doc.querySelector('.description-summary .summary__content, .entry-content .summary__content, .manga-excerpt')?.textContent.trim();
        const posterUrl = doc.querySelector('.summary_image img, .post-thumbnail img, meta[property="og:image"]')?.getAttribute('content') || doc.querySelector('.summary_image img')?.getAttribute('data-src');
