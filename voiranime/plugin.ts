...
async function getHome(cb) {
    try {
        ...
        const results = {};
        const trending = [];
        Array.from(doc.querySelectorAll('.trending-item')).forEach(el => {
            const linkEl = el.querySelector('a');
            const title = el.querySelector('.film-name')?.textContent.trim();
            const url = linkEl?.getAttribute('href');
            const posterUrl = el.querySelector('img')?.getAttribute('data-src');
            if (title && url) {
                trending.append({
                    title,
        ...
        if (trending.length > 0) results['Tendance'] = trending;
...
async function search(query, cb) {
    try {
        ...
        const items = [];
        Array.from(doc.querySelectorAll('.flw-item')).forEach(el => {
            const linkEl = el.querySelector('a');
            const title = el.querySelector('.film-name')?.textContent.trim();
            const url = linkEl?.getAttribute('href');
            const posterUrl = el.querySelector('img')?.getAttribute('data-src');
...
async function load(url, cb) {
    try {
        ...
        const episodes = [];
        const epList = doc.querySelectorAll('.ep-item');
