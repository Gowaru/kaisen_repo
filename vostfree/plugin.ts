...
const baseUrl = 'https://vostfree.com';
const headers = {
...
async function getHome(cb) {
    try {
        ...
        const results = {};
        const items = [];
        Array.from(doc.querySelectorAll('.movie-item')).forEach(el => {
...
async function load(url, cb) {
    try {
        ...
        const html = res.data;
        const doc = await parseHtml(html);
        const title = doc.querySelector('.movie-title')?.textContent.trim();
        const description = doc.querySelector('.movie-desc')?.textContent.trim();
        const posterUrl = doc.querySelector('.movie-poster img')?.getAttribute('src');
        const episodes = [];
        ...
        while ((match = epRegex.exec(html)) !== null) {
            ...
            if (epUrl && epUrl.includes('vostfree')) {
                episodes.push(new Episode({
                    name: epName,
                    episode: parseInt(epName.match(/\d+/) || 1),
