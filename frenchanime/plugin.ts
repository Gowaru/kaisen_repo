...
const baseUrl = 'https://frenchanime.com';
const headers = {
...
async function getHome(cb) {
    try {
        ...
        const results = {};
        const items = [];
        Array.from(doc.querySelectorAll('.film-list .film-item')).forEach(el => {
            const linkEl = el.querySelector('a');
            const imgEl = el.querySelector('img');
            const title = el.querySelector('.film-title')?.textContent.trim();
        ...
        if (items.length > 0) results['Derniers Ajouts'] = items;
...
async function search(query, cb) {
    try {
        const res = await axios.get(`${baseUrl}/search?q=${encodeURIComponent(query)}`, { headers });
        const doc = await parseHtml(res.data);
        const items = [];
        Array.from(doc.querySelectorAll('.film-item')).forEach(el => {
            const linkEl = el.querySelector('a');
            const title = el.querySelector('.film-title')?.textContent.trim();
            const url = linkEl?.getAttribute('href');
...
async function load(url, cb) {
    try {
        ...
        const html = res.data;
        const doc = await parseHtml(html);
        const title = doc.querySelector('.film-info .title')?.textContent.trim();
        const description = doc.querySelector('.description')?.textContent.trim();
        const posterUrl = doc.querySelector('.poster img')?.getAttribute('src');
        const episodes = [];
        const epList = doc.querySelectorAll('.episodes-list a');
        epList.forEach((el, index) => {
...
async function loadStreams(url, cb) {
    try {
        ...
        const html = res.data;
        const streams = [];
        const serverRegex = /<iframe[^>]*src=["']([^"']+)["']/gi;
        let match;
        while ((match = serverRegex.exec(html)) !== null) {
            const embedUrl = match[1];
            if (embedUrl && !embedUrl.includes('ads')) {
